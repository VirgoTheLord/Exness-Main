...
redis.on("message", (channel, message) => {
    ...
    if (channel === "trades") {
        ...
        for (let i = 0; i < data_1.currentOrders.length;) {
            ...
            const id = data_1.currentOrders[i].id;
            ...
            if (order.type === "long") {
                if (order.stopLoss && bid <= order.stopLoss) {
                    ...
                    if (user) {
                        user.balance += pnl + ((_a = order.margin) !== null && _a !== void 0 ? _a : 0);
                    }
                ...
                if (order.takeProfit && bid >= order.takeProfit) {
                    ...
                    if (user) {
                        user.balance += pnl + ((_b = order.margin) !== null && _b !== void 0 ? _b : 0);
                    }
                    data_1.currentOrders.splice(i, 1);
                    continue;
                }
                ...
                if (order.stopLoss && ask >= order.stopLoss) {
                    ...
                    if (user) {
                        user.balance += pnl + ((_c = order.margin) !== null && _c !== void 0 ? _c : 0);
                    }
                    data_1.currentOrders.splice(i, 1);
                    continue;
                }
                if (order.takeProfit && ask <= order.takeProfit) {
                    ...
                    if (user) {
                        user.balance += pnl + ((_d = order.margin) !== null && _d !== void 0 ? _d : 0);
                    }
                    data_1.currentOrders.splice(i, 1);
                    continue;
                }
...
orderRouter.post("/close/:type", (req, res) => {
    ...
    const { id } = (_a = req.user) === null || _a === void 0 ? void 0 : _a.user.id;
    const user = data_1.users.find((d) => d.id === id);
    if (!user) {
